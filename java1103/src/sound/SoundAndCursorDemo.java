...
/**
 * A demo program that can play a few sounds from resource files and can show
 * several different cursors, including a custom cursor that is created from an
 * image resource. A main() routine allows this class to be run as a stand-alone
 * application. (The sound files used by this demo were taken from the KDE
 * desktop.)
 */
public class SoundAndCursorDemo extends JPanel {
	...
	/**
	 * Plays the sound from an audio resource file. If the resource can't be found,
	 * nothing is done. (If the sound is to replayed several times, it would be more
	 * efficient to save the AudioClip in an instance variable instead of loading it
	...
	/**
	 * Sets the current cursor for this component to be a custom cursor defined by
	 * an image resource. If the image resource can't be found, then the component's
	 * cursor is not changed. (For this demo, the hotspot of the cursor is always
	 * set to be the point (7,7).)
	 */
