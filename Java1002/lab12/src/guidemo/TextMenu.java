...
public class TextMenu extends JMenu {
	...
	public TextMenu(DrawPanel owner) {
		...
		lineSpacing.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double lineSpace = panel.getTextItem().getLineHeightMultiplier();
				String s = JOptionPane.showInputDialog(panel, "Multiply the default line spacing by what amount? ",
						lineSpace);
				if (s != null && s.trim().length() > 0) {
					try {
						...
						panel.getTextItem().setLineHeightMultiplier(newLineSpace); // can
						// throw
	...
	// method called makeJustifyMenu() this contains a menu which contains three
	// JRadioButtonMenuItem and are added

	// to the ButtonGroup, add action listeners are added to these three
	// JRadioButtonMenuItem radio buttons.

	private JMenu makeJustifyMenu() {
		JMenu justify = new JMenu("Justify");
		left = new JRadioButtonMenuItem("Left");
		left.setSelected(true);
		// this.justifyTextRadio = left;
